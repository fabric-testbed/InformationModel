...
class AdTest(unittest.TestCase):
    ...
    def testSiteAd(self):
        ...
        gpuw = self.topo.add_node(name='gpuworker', model=worker_model, site=site,
                                        node_id='8CA64353-182D-4CF2-8A07-2ADFF7F9C42A',
                                        ntype=f.NodeType.Server, capacities=gpu_worker_cap)
        fnw = self.topo.add_node(name='fastnetworker', model=worker_model, site=site,
                                            node_id='B87900A2-5225-432A-92F8-60DD961597BE',
                                            ntype=f.NodeType.Server, capacities=network_worker_cap)
        snw = self.topo.add_node(name='slownetworker', model=worker_model, site=site,
        ...
        l10 = self.topo.add_link(name='l9', ltype=f.LinkType.DAC, layer=f.Layer.L2,
                                 interfaces=[self.topo.nodes['slownetworker'].interfaces['snnic1_p2'],
        ...
        l11 = self.topo.add_link(name='l10', ltype=f.LinkType.DAC, layer=f.Layer.L2,
                                 interfaces=[list(filter(lambda iff: (iff.name == 'snnic2_p1'),
                                                         self.topo.interface_list))[0],
                                             switch.interfaces['p10']],
        ...
        _, node_props = self.topo.graph_model.get_node_properties(node_id=switch.interfaces['p9'].node_id)
        self.assertEqual(node_props[ABCPropertyGraph.PROP_LABEL_DELEGATIONS], '[{"pool": "datanic_pool"}]')
        _, node_props = self.topo.graph_model.get_node_properties(node_id=switch.interfaces['p1'].node_id)
